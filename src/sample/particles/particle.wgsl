...
fn vs_main(in : VertexInput) -> VertexOutput {
  ...
  out.position = out.position + vec4<f32>(in.quad_pos * 0.015, 0.0, 0.0);
  out.color = in.color;
...
fn fs_main(in : VertexOutput) -> [[location(0)]] vec4<f32> {
  var color = in.color;
  color.a = color.a * (1.0 - length(in.quad_pos));
  return color;
}
...
[[binding(0), group(0)]] var<uniform> ubo : SimulationParams;
[[binding(1), group(0)]] var<storage, read_write> data : Particles;
[[binding(2), group(0)]] var texture : texture_2d<f32>;

[[stage(compute), workgroup_size(64)]]
fn simulate([[builtin(global_invocation_id)]] GlobalInvocationID : vec3<u32>) {
  rand_seed = (ubo.seed.xy + vec2<f32>(GlobalInvocationID.xy)) * ubo.seed.zw;

  let idx = GlobalInvocationID.x;
  var particle = data.particles[idx];

  // Apply gravity
  particle.velocity.z = particle.velocity.z - ubo.deltaTime * 0.5;

  // Basic velocity integration
  particle.position = particle.position + ubo.deltaTime * particle.velocity;

  // Age each particle. Fade out before vanishing.
  particle.lifetime = particle.lifetime - ubo.deltaTime;
